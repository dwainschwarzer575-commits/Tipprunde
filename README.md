...
<body class="bg-slate-50 text-slate-800 min-h-screen pb-12 selection:bg-indigo-100 selection:text-indigo-900">
    ...
    <div class="relative overflow-hidden bg-white border-b border-slate-200 py-10 px-4 mb-8 shadow-sm">
        ...
        <div class="max-w-6xl mx-auto text-center relative z-10">
            ...
            <p class="text-slate-500 mt-2 text-sm md:text-base max-w-md mx-auto font-medium">
                Tippe gemeinsam mit der Familie! Alle Änderungen synchronisieren sich sofort bei allen Geräten.
            </p>
        </div>
    </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative">
                    <div class="mb-6">
                        ...
                        <input type="text" id="userName" oninput="handleUserChange()" placeholder="Deinen Namen eingeben (schaltet die Tippfelder frei)" class="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-indigo-500 focus:bg-white rounded-xl p-3.5 text-slate-900 placeholder-slate-400 font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-100 transition-all">
                    </div>

                    <!-- Spieletitel -->
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                        <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Spielpaarungen &amp; Ergebnisse</span>
                        <span id="syncIndicator" class="text-[11px] text-emerald-600 font-semibold flex items-center gap-1 bg-emerald-50 px-2.5 py-1 rounded-full">
                            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> Echtzeit-Autosave aktiv
                        </span>
                    </div>
                    ...
                    <!-- Manueller Backup-Speicher-Button -->
                    <button onclick="saveMyTips()" class="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white p-4 rounded-xl mt-6 font-bold tracking-wide shadow-sm hover:shadow transition-all flex items-center justify-center gap-2">
                        ...
                        Tipps &amp; Ergebnisse manuell sichern
                    </button>
                </div>
            </div>
    ...
    <script type="module">
        ...
        // 1. INITIALE LOCAL-FIRST DATEN (Schneller Start in unter 1ms!)
        const initialGames = [
        ...
        // Sofortiges Rendern der Offline-Daten
        render();
        renderLeaderboard();

        // 2. CHECK OB FIREBASE CONFIG EXISTIERT
        ...
        try {
            ...
            console.warn("Cloud-Dienste nicht initialisiert (Lokaler Modus aktiv).", e);
        }
        ...
        if (!hasCloudConfig) {
            ...
            syncIndicator.innerHTML = `<span class="w-2 h-2 rounded-full bg-indigo-400"></span> Lokale Speicherung`;
            syncIndicator.className = "text-[11px] text-indigo-600 font-semibold flex items-center gap-1 bg-indigo-50 px-2.5 py-1 rounded-full";
        } else {
            connectionStatus.innerText = "🔄 Verbinde im Hintergrund mit der Cloud...";
            initAuth();
        }

        // 3. CLOUD-AUTHENTIFIZIERUNG IM HINTERGRUND
        ...
        // 4. ECHTZEIT-SYNCHRONISATION (LIVE-LISTENERS)
